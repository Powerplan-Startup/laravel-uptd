<div class="my-3 p-3 bg-body rounded shadow-sm">
    @if($paket->count() <= 0)
        <h5 class="border-bottom pb-2 mb-0">Belum Ada Paket</h5>
            ...
            <div class="pb-3 mb-0 small lh-smw-100">
                <div class="d-flex justify-content-between">
                    <div class="alert alert-warning">
                        Anda belum menjadi menginstruksi paket apapun
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($paket as $item)
                            <tr>
                                ...
                                <td> <kbd>{{ optional($item->kejuruan)->nama_kejuruan }}</kbd> {{ $item->paket }}/{{ $item->tahun }}</td>
                                <td>
                                    {{ $item->peserta_count }}
                                </td>
                                <td>
                                    {{ $item->nilai_count }} / {{ $item->peserta_count }}
                                </td>
                                <td>
                                    @if(request()->route('paket') && request()->route('paket') == $item->id_paket)
                                        <button class="btn btn-success btn-sm d-block w-100" type="submit">
                                            Simpan
                                        </button>
                                    @else
                                        <a href="{{ route('instruktur.nilai', ['paket' => $item->id_paket]) }}" class="btn btn-primary btn-sm">
                                            Input Nilai
                                        </a>
                                    @endif
                                </td>
