        ...
        <v-dialog v-model="dialog" max-width="400" content-class="shadow-sm" overlay-opacity=".25" eager scrollable persistent>
            <v-form @submit.prevent="submit" :disabled="loading">
                <v-card>
                    <v-toolbar flat>
                        <v-subheader>
                            Konfirmasi Hapus data Instruktur > {{ item.nama_instruktur }}
                        </v-subheader>
                        <v-spacer/>
...
export default {
    ...
    computed: {
        ...mapState({
            value_dialog: state => state.instruktur.modal.hapus,
    ...
    methods: {
        ...mapActions({
            setDialog: 'instruktur/setModalHapus',
            deleteInstruktur: 'instruktur/destroy',
        ...
        async submit(e){
            ...
            this.loading = true
            let res = await this.deleteInstruktur({ data, id: this.id }).catch(e => {
                console.log("deleteInstruktur@InstrukturTambah.vue", e);
        ...
        async loadItem(){
            ...
            let res = await this.findInstruktur({ id: this.id }).catch(e => {
                console.log("loadItem@InstrukturTambah.vue", e);
                this.notif({
                    message: e.message
                })
            })
