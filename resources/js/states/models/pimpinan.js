...
export default {
    ...
    actions: {
        ...
        count(context, params = {}){
            return new Promise(async(resolve, reject)=>{
                let res = await axios.get(api('analytic/pimpinan/count'), { params: params }).catch(e => reject(e))
                if(res) resolve(res)
            })
        },
        async show(context, {data = {}, id}){
            if(id)
                return new Promise(async(resolve, reject)=>{
                    let res = await axios.get(api(`pimpinan/${id}`), { params: data }).catch(e => reject(e))
                    if(res) resolve(res)
                })
            else
